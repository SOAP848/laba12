...
from app.dependencies.auth import require_admin, require_restaurant_manager, get_current_user
...
def create_dish(
    ...
    restaurant = db.query(Restaurant).filter(Restaurant.id == dish_data.restaurant_id).first()
    if not restaurant:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
