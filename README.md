...
3. Скрипт `scripts/ai_review.py` получает diff изменений, анализирует его с помощью OpenAI API и публикует структурированный комментарий в PR.
...
1. Сгенерированы недостающие unit-тесты для всех сервисов (`DishService`, `OrderService`, `FavoriteService`, `ReviewService`), покрывающие основные сценарии, граничные случаи и обработку ошибок.
2. Добавлены тесты для задач Celery (`email_tasks.py`).
